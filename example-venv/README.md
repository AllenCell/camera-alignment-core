...
* `.python-version`: Tells tools like `uv` or `pyenv` which python version to use. This is different from the `requires-python` line in `pyproject.toml`, which is set to a very general `>=3.8`. Together, these mean that the example project *should* work on Python 3.8+, but we're using the version specified in `.python-version`.
