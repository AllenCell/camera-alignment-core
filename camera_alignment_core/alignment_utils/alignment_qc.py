...
class AlignmentQC:
    def __init__(
        ...
        """
        Initialization of all variables needed for QC

        -reference: fixed channel image
        -moving: moving channel image
        -reference_seg: fixed channel segmentation
        -moving_seg: moving channel segmentation
        -ref_mov_coor_dict: dictionary matching reference and moving bead coordinates prior to alignment formated as {(y1,x1): (y2,x2)}
        -tform: calculated similarity transform for images
        """
    ...
    def report_full_metrics(
        ...
        """
        Report all programmed metrics. Requires all variables to be defined
        """
    ...
    def check_z_offset_between_ref_mov(self) -> Tuple[int, int, int]:
        """
        Calculate the difference in between z-midpoints for bead arrays
        """
        ...
        # TODO: Make test with handbuilt numpy arrays
    ...
    def report_ref_mov_image_snr(self) -> Tuple[int, int, int, int]:
        """
        Calculate signal and noise of reference and moving images
        """
        ...
        def get_image_snr(
            ...
            if seg is None or img_intensity is None:
                raise ValueError("Segmentation and raw image must be defined")
    ...
    def report_number_beads(self) -> Tuple[bool, int]:
        """
        Report number of beads matched and whether it's greater than 10
        """
    ...
    def report_change_fov_intensity_parameters(self) -> Dict[str, int]:
        ...
        # TODO think of how to test this
        change_fov_intensity_param_dict = {
            "median_intensity": np.median(self.moving_transformed)
            - np.median(self.moving_source[self.mov_origin]),
            "min_intensity": np.min(self.moving_transformed)
            - np.min(self.moving_source[self.mov_origin]),
            "max_intensity": np.max(self.moving_transformed)
            - np.max(self.moving_source[self.mov_origin]),
            "1st_percentile": np.percentile(self.moving_transformed, 1)
            - np.percentile(self.moving_source[self.mov_origin], 1),
            "995th_percentile": np.percentile(self.moving_transformed, 99.5)
    ...
    def report_changes_in_coordinates_mapping(self) -> Tuple[bool, float]:
        ...
        y_size = int(self.reference[self.ref_origin].shape[0] / 3)
        x_size = int(self.reference[self.ref_origin].shape[1] / 3)
        ...
        dist_after_center = []
        for bead in range(0, len(mov_coors)):
            if (y_lim[1] > mov_coors[bead][0]) and (mov_coors[bead][0] > y_lim[0]):
                if (x_lim[1] > mov_coors[bead][1]) and (mov_coors[bead][1] > x_lim[0]):
